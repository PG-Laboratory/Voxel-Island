const Sprocket = function (canvasElement, antialias, alpha) {
    ...
    this.Surface = function () {
        ...
        let _width = 0;
        let _height = 0;
        let _clearColor = new Sprocket.Color(1, 1, 1, 0);
        let _linear = false;
        let _repeat = false;
    ...
    this.Shader = function (fragment, surfaces, variables) {
        ...
        const makeVariablesOut = () => {
            ...
            for (const variable of _shaderVariables)
                if (fragment.includes(variable.name))
    ...
    const pushIdentity = () => {
        if (++_transformAt === _transformStack.length)
            _transformStack.push(new Sprocket.Transform());
        else
    ...
    const _sprites = [];
    const _transformStack = [new Sprocket.Transform(1, 0, 0, 0, -1, canvasElement.height)];
...
Sprocket.Color = function (r, g, b, a) {
    this.r = r;
    this.g = g;
    this.b = b;
...
Sprocket.Color.BLACK = new Sprocket.Color(0, 0, 0);
Sprocket.Color.BLUE = new Sprocket.Color(0, 0, 1);
Sprocket.Color.GREEN = new Sprocket.Color(0, 1, 0);
Sprocket.Color.CYAN = new Sprocket.Color(0, 1, 1);
Sprocket.Color.RED = new Sprocket.Color(1, 0, 0);
Sprocket.Color.MAGENTA = new Sprocket.Color(1, 0, 1);
Sprocket.Color.YELLOW = new Sprocket.Color(1, 1, 0);
Sprocket.Color.WHITE = new Sprocket.Color(1, 1, 1);

Sprocket.Color.fromHex = hex => {
    const integer = parseInt(hex, 16);

    if (hex.length === 6)
        return new Sprocket.Color(
            ((integer >> 16) & 0xFF) / 255,
            ((integer >> 8) & 0xFF) / 255,
            (integer & 0xFF) / 255);
    else
...
Sprocket.Color.prototype.toHex = function () {
...
Sprocket.Color.fromHSV = (h, s, v) => {
    ...
    switch (Math.floor(h * 6)) {
        case 1:
            return new Sprocket.Color(x + m, c + m, m);
        case 2:
            return new Sprocket.Color(m, c + m, x + m);
        case 3:
            return new Sprocket.Color(m, x + m, c + m);
        case 4:
            return new Sprocket.Color(x + m, m, c + m);
        case 5:
            return new Sprocket.Color(c + m, m, x + m);
        default:
            return new Sprocket.Color(c + m, x + m, m);
    }
};

Sprocket.Color.prototype.toHSV = function () {
...
Sprocket.Color.prototype.copy = function () {
    return new Sprocket.Color(this.r, this.g, this.b, this.a);
};

Sprocket.Color.prototype.add = function (color) {
...
Sprocket.Color.prototype.multiply = function (color) {
    this.r *= color.r;
    this.g *= color.g;
    this.b *= color.b;
...
Sprocket.Color.prototype.equals = function (color) {
    return this.r === color.r && this.g === color.g && this.b === color.b && this.a === color.a;
};

Sprocket.Vector = function (x, y) {
    this.x = x;
    this.y = y;
};

Sprocket.Vector.prototype.copy = function () {
    return new Sprocket.Vector(this.x, this.y);
};

Sprocket.Vector.prototype.add = function (vector) {
    this.x += vector.x;
    this.y += vector.y;
};

Sprocket.Vector.prototype.subtract = function (vector) {
    this.x -= vector.x;
    this.y -= vector.y;
};

Sprocket.Vector.prototype.negate = function () {
    this.x = -this.x;
    this.y = -this.y;
};

Sprocket.Vector.prototype.dot = function (vector) {
    return this.x * vector.x + this.y * vector.y;
};

Sprocket.Vector.prototype.length = function () {
    return Math.sqrt(this.dot(this));
};

Sprocket.Vector.prototype.multiply = function (scalar) {
    this.x *= scalar;
    this.y *= scalar;
};

Sprocket.Vector.prototype.divide = function (scalar) {
    if (scalar === 0)
        this.x = this.y = 0;
    else
...
Sprocket.Vector.prototype.normalize = function () {
    this.divide(this.length());
};

Sprocket.Vector.prototype.angle = function () {
    return Math.atan2(this.y, this.x);
};

Sprocket.Vector.prototype.reflect = function (vector) {
...
Sprocket.Vector.prototype.equals = function (vector) {
    return this.x === vector.x && this.y === vector.y;
};

Sprocket.Vector.prototype.rotate = function (angle) {
    const cos = Math.cos(angle);
    const sin = Math.sin(angle);
    const x = this.x;
    const y = this.y;
...
Sprocket.Transform = function (_00, _10, _20, _01, _11, _21) {
    if (_00 === undefined)
        this.identity();
    else {
        this._00 = _00;
...
Sprocket.Transform.prototype.apply = function (vector) {
    const x = vector.x;
    const y = vector.y;
...
Sprocket.Transform.prototype.copy = function () {
    return new Sprocket.Transform(this._00, this._10, this._20, this._01, this._11, this._21);
};

Sprocket.Transform.prototype.identity = function () {
    this._00 = 1;
    this._10 = 0;
    this._20 = 0;
    this._01 = 0;
...
Sprocket.Transform.prototype.set = function (transform) {
...
Sprocket.Transform.prototype.multiply = function (transform) {
    const _00 = this._00;
    const _10 = this._10;
    const _01 = this._01;
    const _11 = this._11;
...
Sprocket.Transform.prototype.rotate = function (angle) {
    const cos = Math.cos(angle);
    const sin = Math.sin(angle);
...
Sprocket.Transform.prototype.shear = function (x, y) {
    const _00 = this._00;
    const _01 = this._01;
...
Sprocket.Transform.prototype.translate = function (x, y) {
    this._20 += this._00 * x + this._10 * y;
    this._21 += this._01 * x + this._11 * y;
};

Sprocket.Transform.prototype.scale = function (x, y) {
    this._00 *= x;
    this._10 *= y;
    this._01 *= x;
    this._11 *= y;
...
Sprocket.Transform.prototype.invert = function () {
    const s11 = this._00;
...
if (typeof module !== 'undefined') module.exports = Sprocket;